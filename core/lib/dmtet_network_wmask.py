...
import random
import trimesh
from .encodings import HashEncodingWMask
...
class HashDecoder(nn.Module):
    def __init__(self, input_dims = 3, internal_dims = 32, output_dims = 4, hidden = 2, input_bounds=None, max_res=1024, num_levels=16) -> None:
        ...
        self.embed_fn = HashEncodingWMask(implementation='torch', max_res=max_res, num_levels=num_levels)
...
class DMTetMeshWMask(nn.Module):
    ...
    def query_decoder(self, tet_v, num_unmask):
        if self.tet_v.shape[0] < 1000000:
            zero_mask = torch.zeros((tet_v.shape[0], self.hash_num_levels, 1), dtype=torch.float32, device=self.device)
            zero_mask[:, :8+num_unmask] = 1
            return self.decoder(tet_v, zero_mask)
        else:
            chunk_size = 1000000
            results = []
            for i in range((tet_v.shape[0] // chunk_size) + 1):
                ...
                if i*chunk_size < tet_v.shape[0]:
                    tet_v_now = tet_v[i*chunk_size: (i+1)*chunk_size]
                    zero_mask = torch.zeros((tet_v_now.shape[0], self.hash_num_levels, 1), dtype=torch.float32, device=self.device)
                    zero_mask[:, :8+num_unmask] = 1
                    results.append(
                        self.decoder(tet_v_now, zero_mask)
                    )

            return torch.cat(results, dim=0)

    def get_mesh(self, return_loss=False, num_subdiv=None, global_step=0):

        # zero_mask = torch.zeros((self.tet_v.shape[0], self.hash_num_levels, 1), dtype=torch.float32, device=self.device)
        # zero_mask = self.get_num_mask(global_step, zero_mask)
        num_unmask = self.get_num_mask(global_step)
        ...
        if self.use_explicit:
            ...
            pred = self.query_decoder(self.tet_v, num_unmask)
        ...
        for i in range(num_subdiv):
            ...
            tet = tet[0]
            pred = self.query_decoder(verts_deformed, num_unmask)
            sdf, _ = pred[:,0], pred[:,1:]
        ...
        if return_loss:
            zero_mask = torch.zeros((self.tet_v.shape[0], self.hash_num_levels, 1), dtype=torch.float32, device=self.device)
            zero_mask[:, :8+num_unmask] = 1
            gradients = self.decoder.gradient(self.tet_v, zero_mask)
            gradients_error = (torch.linalg.norm(gradients.reshape(1, -1, 3), ord=2,
                                            dim=-1) - 1.0) ** 2
            return mesh_verts, mesh_faces, gradients_error
        else:
    ...
    def init_mesh(self, mesh_v, mesh_f, init_padding=0.):
        ...
        '''
        self.tet_v torch.Size([646083, 3]) torch.return_types.max(
        values=tensor([0.2587, 0.6009, 0.2659], device='cuda:0'),
        indices=tensor([218, 429, 201], device='cuda:0')) torch.return_types.min(
        values=tensor([-0.2569, -0.6002, -0.2656], device='cuda:0'),
        indices=tensor([221,   6,  69], device='cuda:0'))
        '''
        ...
        if self.use_explicit:
            ...
            for i in tqdm(range(iter)):
                ...
                zero_mask = torch.zeros((p.shape[0], self.hash_num_levels, 1), dtype=torch.float32, device=self.device)
                zero_mask[:, :8] = 1
        
                pred = self.decoder(p, zero_mask)
                sdf, deform = pred[:,0], pred[:,1:]
    ...
    def get_num_mask(self, global_step):
        unmask_steps = [500*i for i in range(8)]
        num_unmask = 0
        for step in unmask_steps:
            if global_step >= step:
                num_unmask += 1
